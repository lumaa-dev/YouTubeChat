...
public class YouTubeChatService implements ChatService
{
        ...
        this.executor.execute(() ->
        {
            try
            {
                ...
                for (LiveBroadcast broadcast : broadcastList.execute().getItems())
                {
                    ...
                    {
                        LoggerYT.info("Live Chat ID: {}", this.getLiveChatId());
                        break;
                    }
                }
                ...
                if (this.getLiveChatId() == null || this.getLiveChatId().isEmpty())
                {
                    LoggerYT.printExceptionMessage("Couldn't find live stream on current channel");
                    this.stop(false);
                    return;
                }
    ...
    public void stop(boolean logout)
    {
        ...
        if (this.failed)
        {
            LoggerYT.printYTMessage(JsonUtils.create("Service stopped due to error").setStyle(JsonUtils.red()));
        }
        else
        {
            LoggerYT.printYTMessage(JsonUtils.create(logout ? "Stopped service and logout" : "Service stopped").setStyle(JsonUtils.green()));
        }
    }
    ...
    private void printGoogleJsonException(GoogleJsonResponseException e)
    {
        try
        {
            ...
            LoggerYT.printExceptionMessage("Error Code: " + googleEx.getErrorCode());
            LoggerYT.printExceptionMessage("Message: " + googleEx.getMessage());
        }
