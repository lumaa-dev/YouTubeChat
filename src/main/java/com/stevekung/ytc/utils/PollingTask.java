    ...
    public void run()
    {
        try
        {
            ...
            for (LiveChatMessage message : chatResponse.getItems())
            {
                ...
                service.getListener().onChatReceived(new ChatReceivedEvent(message.getAuthorDetails(), snippet.getSuperChatDetails(), message.getId(), TextFormatting.getTextWithoutFormattingCodes(snippet.getDisplayMessage()), moderatorId));
            }
